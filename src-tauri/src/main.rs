...
async fn cpu_mem() -> String{
    unsafe{
        if let Some(my_boxed_session) = GLOBAL_STRUCT.as_ref(){
            let mut channel = (my_boxed_session).open_session.channel_session().unwrap();
            channel.exec("free | grep Mem | awk '{print $3/$2 * 100.0}' ; ps aux --sort -%cpu | head -10 | awk '{ total += $3} END {print total}'").unwrap();
            let mut s = String::new();
            channel.read_to_string(&mut s).unwrap();
            println!("{}",s);
            s
    ...
    unsafe{
        if GLOBAL_STRUCT.is_some(){
            ...
            if let Some(my_boxed_session) = GLOBAL_STRUCT.as_ref(){
                ...
                let mut channel = (my_boxed_session).open_session.channel_session().unwrap();
                println!("arrived here.");
                let command: String = format!("echo 'export MONIKER= {moniker_name}' >> $HOME/.bash_profile; wget -O lava.sh https://node101.io/testnet/lava/lava.sh && chmod +x lava.sh && bash lava.sh ");
                channel.exec(&*command).unwrap();
                let mut s = String::new();
                channel.read_to_string(&mut s).unwrap();
                if s.contains("SETUP IS FINISHED"){
    ...
    unsafe{
        if GLOBAL_STRUCT.is_some(){
            ...
            if let Some(my_boxed_session) = GLOBAL_STRUCT.as_ref(){
                ...
                let mut channel = (my_boxed_session).open_session.channel_session().unwrap();
                println!("arrived here.");
                let command: String = format!("export PATH=$PATH:/usr/local/go/bin:/root/go/bin; lavad keys list --output json;");
                channel.exec(&*command).unwrap();
                let mut s = String::new();
                channel.read_to_string(&mut s).unwrap();
                println!("{}",s);
    ...
    unsafe{
        if GLOBAL_STRUCT.is_some(){
            ...
            if let Some(my_boxed_session) = GLOBAL_STRUCT.as_ref(){
                ...
                let mut channel = (my_boxed_session).open_session.channel_session().unwrap();
                println!("arrived here.");
                let command: String = format!("export PATH=$PATH:/usr/local/go/bin:/root/go/bin; lavad keys delete {wallet_name} --output json;");
                channel.exec(&*command).unwrap();
                let mut s = String::new();
                channel.read_to_string(&mut s).unwrap();
                if(s.contains(":")){
...
async fn unjail(node_name:String,wallet_name:String,password:String,fees:String) -> String {
    unsafe{
        if let Some(my_boxed_session) = GLOBAL_STRUCT.as_ref(){
            let mut channel = (my_boxed_session).open_session.channel_session().unwrap();
            channel.exec(r"{node_name} tx slashing unjail \
...
fn main() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![log_in,log_out,cpu_mem,node_info,am_i_logged_out,current_node,install_node,unjail,create_wallet,show_wallets,delete_wallets])
