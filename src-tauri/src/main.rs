...
async fn validator_info() {
    unsafe {
        if let Some(my_boxed_session) = GLOBAL_STRUCT.as_ref() {
            ...
            channel
                .exec(
                    "export PATH=$PATH:/usr/local/go/bin:/root/go/bin; lavad status 2>&1 | jq .ValidatorInfo"
                )
                .unwrap();
            let mut s = String::new();
            channel.read_to_string(&mut s).unwrap();
...
async fn node_info() -> String {
    unsafe {
        if let Some(my_boxed_session) = GLOBAL_STRUCT.as_ref() {
            let mut channel = my_boxed_session.open_session.channel_session().unwrap();
            channel
                .exec("export PATH=$PATH:/usr/local/go/bin:/root/go/bin; lavad status 2>&1")
                .unwrap();
            let mut s = String::new();
            channel.read_to_string(&mut s).unwrap();
            s
...
async fn install_node(
    ...
    unsafe {
        if let Some(my_boxed_session) = GLOBAL_STRUCT.as_ref() {
            ...
            channel
                .exec(
                    "echo 'export MONIKER=' {moniker_name} >> $HOME/.bash_profile; wget -O {node_name}.sh https://node101.io/mainnet/{node_name}.sh && chmod +x {node_name}.sh && ./{node_name}.sh"
                )
                .unwrap();
...
async fn create_wallet(wallet_name: String, password: String) -> Result<String, String> {
    Ok(
        "Will return mnemonics if created first, if not then will return a success or anything.".into()
    )
}
...
fn main() {
    tauri::Builder
        ...
        .invoke_handler(
            tauri::generate_handler![log_in, logout, cpu_mem, node_info, am_i_logged_out]
        )
